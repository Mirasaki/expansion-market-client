...
const {
  MARKET_TRADER_MAPS_FILE_DESCRIPTION, CONFIRMATION_PROMPT_OPTION_NAME, CONFIRMATION_PROMPT_OPTION_DESCRIPTION
} = require('../../constants');
...
module.exports = new ChatInputCommand({
  ...
  run: async (client, interaction) => {
    ...
    if (confirmationPrompt !== true) {
      interaction.followUp({ content: `${ emojis.error } ${ member }, you didn't select **\`true\`** on the confirmation prompt, this command has been cancelled.` });
    ...
    if (res.status === 200) {
      interaction.followUp({ content: `${ emojis.success } ${ member } - ${ res.message } (usually deleted by relation/association to traders and trader-zones)` });
    }

    // 404 - Not Found
    else if (res.status === 404) {
      interaction.followUp({ content: `${ emojis.error } ${ member }, there is no ${ MARKET_TRADER_MAPS_FILE_DESCRIPTION } active for this server.` });
    }
